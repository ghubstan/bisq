...
public class BotClient {
    ...
    public boolean takeOfferFailedForOneOfTheseReasons(Throwable t, String... reasons) {
        if (t == null)
            throw new IllegalArgumentException("Throwable argument cannot be null.");

        if (reasons == null || reasons.length == 0)
            throw new IllegalArgumentException("Reasons argument cannot be null or empty.");

        return stream(reasons).anyMatch(reason -> reason.equals(t.getMessage()));
    }
